...
Vendored from `ActiveVisionLab/SD4Match` and kept close to upstream behavior.
"""
...
from ..utils.geometry import regularise_coordinates
...
class SPairDataset(CorrespondenceDataset):
    def __init__(self, cfg, split, category="all"):
        ...
        self.src_identifiers = [
            f"{self.cls[ids]}-{name[:-4]}" for ids, name in zip(self.cls_ids, self.src_imnames)
        ]
        self.trg_identifiers = [
            f"{self.cls[ids]}-{name[:-4]}" for ids, name in zip(self.cls_ids, self.trg_imnames)
        ]
    ...
    def __getitem__(self, idx):
        ...
        # get pckthres
        sample["trg_pckthres"] = self.get_pckthres({"trg_img": sample["trg_img"], "trg_bbox": sample["trg_bbox"]})
        sample["src_pckthres"] = self.get_pckthres({"trg_img": sample["src_img"], "trg_bbox": sample["src_bbox"]})
    ...
    def get_mask(self, sample, imname, scaled_imsize):
        ...
        tensor_mask = (
            F.interpolate(
                tensor_mask.unsqueeze(0).unsqueeze(0).float(),
...
class SPairImageDataset(Dataset):
    def __init__(self, cfg, split, category, transform=None, tokenizer=None):
        ...
        self.impaths = list(
            map(lambda x: os.path.join(self.root, "JPEGImages", get_name(x)) + ".jpg", unique)
        )
        self.annofiles = list(
            map(lambda x: os.path.join(self.root, "ImageAnnotation", get_name(x)) + ".json", unique)
        )
        ...
        self.bbox = list(map(lambda x: torch.tensor(x["bndbox"]).float(), anno_data))
    ...
    def __getitem__(self, index):
        ...
        # NOTE: kept close to upstream behavior (imports in upstream used plt.imread).
        img = np.array(Image.open(self.impaths[index]).convert("RGB"))
        img = np.copy(img)
        ...
        output.update(
            {"identifier": self.imnames[index], "impath": self.impaths[index], "category": category}
        )
