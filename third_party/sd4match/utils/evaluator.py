"""SD4Match PCK evaluator (vendored)."""

import numpy as np
import torch

from .geometry import scaling_coordinates
...
class PCKEvaluator:
        ...
        for b in range(B):
            ...
            for alpha in self.alpha:
                if self.by == "image":
                    ...
                    raise ValueError("select between ('image', 'point')")
    ...
    def evaluate_feature_map(
        ...
        alpha[float]
        softmax_temp[float]: softmax temperature for matching_strategy being softargmax.
        """
        ...
        bilinear_matches = bilinear_get_matches(
            src_featmaps, trg_featmaps, src_kps, l2_norm=enable_l2_norm
        )
        nn_matches = nn_get_matches(src_featmaps, trg_featmaps, src_kps, l2_norm=enable_l2_norm)
        softargmax_matches = softargmax_get_matches(
            src_featmaps, trg_featmaps, src_kps, softmax_temp, l2_norm=enable_l2_norm
        )
        kernelsoftargmax_matches = kernel_softargmax_get_matches(
            src_featmaps,
            trg_featmaps,
            src_kps,
            softmax_temp,
            gaussian_suppression_sigma,
