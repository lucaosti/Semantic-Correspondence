"""SD4Match matching utilities (vendored)."""
...
from .misc import softmax_with_temperature
...
def bilinear_get_matches(src_featmaps, trg_featmaps, query, l2_norm=True):
    ...
    i_m_m = torch.cat(
        (torch.floor(query[..., 0:1]), torch.floor(query[..., 1:2])), dim=-1
    )  # index_minus_minus (x, y)
    i_m_p = torch.cat(
        (torch.floor(query[..., 0:1]), torch.floor(query[..., 1:2]) + 1), dim=-1
    )  # index_minus_plus
    i_p_m = torch.cat(
        (torch.floor(query[..., 0:1]) + 1, torch.floor(query[..., 1:2])), dim=-1
    )  # index_plus_minus
    i_p_p = torch.cat(
        (torch.floor(query[..., 0:1]) + 1, torch.floor(query[..., 1:2]) + 1), dim=-1
    )  # index_plus_plus
    ...
    matches = (Q_m_m * f_m_m + Q_p_p * f_p_p + Q_m_p * f_m_p + Q_p_m * f_p_m) / (
...
def softargmax_get_matches(src_featmaps, trg_featmaps, query, softmax_temp, l2_norm=True):
    ...
    softmax_temp [float] : temperature for softmax.
    OUT:
