...
In our implementation, we applied manual LoRA adapters exclusively to the Multilayer Perceptron (MLP) linear layers of the final Transformer blocks. To study the sensitivity of this approach to adaptation depth, we sweep the number of LoRA-equipped blocks over $N \in \{1, 2, 4\}$, mirroring the fine-tuning depth ablation of Phase~2. This surgical intervention drastically reduces the number of trainable parameters while giving the model enough flexibility to adapt to the semantic correspondence task on the SPair-71k dataset.