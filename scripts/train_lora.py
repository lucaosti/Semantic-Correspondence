    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            if "full_epochs_done" in blob:
                ...
                batch_in_epoch = 0
            print(
                f"train_lora: RESUME full_epochs_done={full_epochs_done} batch_in_epoch={batch_in_epoch} "
                f"(file={args.resume} best_val={best_val})",
                flush=True,
            )
            if batch_in_epoch:
                print(
                    f"train_lora: mid-epoch resume; skipping first {batch_in_epoch} batches "
    ...
    for epoch in range(full_epochs_done, args.epochs):
        ...
        for batch_idx, batch in enumerate(train_it, start=skip_batches):
            ...
            if iv > 0 and (done_in_epoch % iv == 0 or done_in_epoch == n_train_batches):
                ...
                print(
                    f"train_lora: saved resume checkpoint (batch_in_epoch={done_in_epoch}/{n_train_batches}) -> {resume_path}",
                    flush=True,
                )
